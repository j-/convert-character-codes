...
// https://github.com/WebKit/WebKit/blob/f9b6f92885015e79ba95513019cdd7248f62972c/Source/WebCore/html/parser/HTMLEntityNames.in
const htmlEntityNamesAll = new Map([
  ["AElig;","U+000C6"],
  ["AElig","U+000C6"],
  ["AMP;","U+00026"],
  ["AMP","U+00026"],
...
export const mapCodePointByEntity = new Map(
  Array.from(htmlEntityNamesAll.entries())
    // Remove redundant name entries.
    .filter(([name]) => name.endsWith(';'))
    // Map to "&" <name> ";".
    .map(([name, value]) => [`&${name}`, value])
    // Remove entries with multiple codepoints.
    .filter(([, value]) => !/ /.test(value))
    // Remove entries not recognised by this device.
    .filter((entry): entry is [string, string] => {
      const [name] = entry;
      testElement.innerHTML = name;
      // console.debug(name, testElement.innerHTML, testElement.textContent);
      const isRecognised = testElement.textContent !== name;
      if (!isRecognised) {
        console.debug('Did not recognise', name);
      }
      return isRecognised;
    })
    // Convert value from unicode to codepoint.
    .map(([name, value]) => [name, codecUnicode.decode(value)])
);

export const mapEntityByCodePoint = new Map(
  Array.from(mapCodePointByEntity.entries(), ([name, value]) => [value, name])
);

export const entityNames = [...mapCodePointByEntity.keys()];

export const getEntitiesForCodePoint = (codePoint: number) => (
  new Set(
    Array.from(mapCodePointByEntity.entries())
      .filter(([_, value]) => value === codePoint)
      .map(([name]) => name)
  )
);
